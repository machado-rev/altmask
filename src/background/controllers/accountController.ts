...
import { Wallet as QtumWallet } from 'althashjs-wallet';
import assert from 'assert';

import AltmaskController from '.';
import IController from './iController';
import { MESSAGE_TYPE, STORAGE, NETWORK_NAMES, ALTMASK_ACCOUNT_CHANGE } from '../../constants';
...
export default class AccountController extends IController {
  ...
  */
  public saveToFile = (accountName: string, mnemonic: string) => {
    ...
    element.download = `altmask_${accountName}_${timestamp}.bak`;
    element.click();
  ...
  */
  public onAccountLoggedIn = async (isSessionRestore = false) => {
     ...
     * reopening the popup, we don't need to send the SEND_INPAGE_ALTMASK_ACCOUNT_VALUES event to
     * the inpage because window.altmask.account has not changed.
     */
    if (!isSessionRestore) {
      this.main.inpageAccount.sendInpageAccountAllPorts(ALTMASK_ACCOUNT_CHANGE.LOGIN);
    }
  ...
  */
  private getWalletInfo = async (sendInpageUpdate = true) => {
    ...
    if (infoDidUpdate) {
      ...
      if (sendInpageUpdate) {
        this.main.inpageAccount.sendInpageAccountAllPorts(ALTMASK_ACCOUNT_CHANGE.BALANCE_CHANGE);
      }
  ...
  * @param receiverAddress The address to send Html to.
  ...
  */
  private sendTokens = async (receiverAddress: string, amount: number, transactionSpeed: TRANSACTION_SPEED) => {
    ...
    */
    try {
      ...
      const feeRate = rates[transactionSpeed]; // satoshi/byte; 500 satoshi/byte == .005 HTML/KB
      if (!feeRate) {
   ...
   * For instance when a user sends Html, but that transaction has not confirmed yet,
