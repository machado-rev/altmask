...
export function showModal(width: number, height: number, style: any = {}, src?: string): Promise<HTMLIFrameElement> {
  ...
  addClass(iframe, 'altmask-extension-modal');

  const backdrop = document.createElement('div');
  addClass(backdrop, 'altmask-extension-modal-backdrop');

  const html = document.documentElement;
  addClass(html, 'altmask-extension-modal-wrap');
...
export function closeModal() {
  const iframe = document.querySelector('iframe.altmask-extension-modal');

  if (iframe) {
    iframe.remove();
    const backdrop = document.querySelector('.altmask-extension-modal-backdrop');

    if (backdrop) { backdrop.remove(); }

    removeClass(document.documentElement, 'altmask-extension-modal-wrap');
  }
}
